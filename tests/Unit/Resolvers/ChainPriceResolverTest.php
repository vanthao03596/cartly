...
class ChainPriceResolverTest extends TestCase
{
    ...
    public function test_it_uses_first_successful_resolver(): void
    {
        $item = new CartItem(id: 1, quantity: 1);
        $context = $this->createContext();

        $resolver1 = $this->createFakeResolver(1000);
        $resolver2 = $this->createFakeResolver(2000);

        $chain = new ChainPriceResolver([$resolver1, $resolver2]);
        $result = $chain->resolve($item, $context);

        $this->assertSame(1000, $result->unitPrice);
    }

    public function test_it_falls_back_to_next_resolver_on_failure(): void
    {
        $item = new CartItem(id: 1, quantity: 1);
        $context = $this->createContext();
        ...
        $this->assertSame(2000, $result->unitPrice);
    }
    ...
    public function test_it_throws_when_no_resolvers(): void
    {
        ...
        $chain = new ChainPriceResolver([]);

        $this->expectException(UnresolvablePriceException::class);
        $chain->resolve($item, $context);
    }

    public function test_it_resolves_many_items(): void
    {
        ...
        $resolver = $this->createFakeResolver(500);
        $chain = new ChainPriceResolver([$resolver]);

        $results = $chain->resolveMany($items, $context);

        $this->assertCount(2, $results);
        $this->assertSame(500, $results[$item1->rowId]->unitPrice);
        $this->assertSame(500, $results[$item2->rowId]->unitPrice);
    }
